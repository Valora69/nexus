...
import { Payment, PaymentWithRelations } from '../types/entities';
...
export const createPayment = async (
  ...
  const data = await http(`${BASE}${PAYMENT_URI}`, {
    method: 'POST',
...
> => {
  const data = await http(`${BASE}${PAYMENT_URI}/pending-verification`, {
    method: 'GET',
    credentials: 'include',
    headers: {
      'Content-Type': 'application/json',
...
export const getPendingConfirmation = async (): Promise<
  PaymentWithRelations[]
> => {
  const data = await http(`${BASE}${PAYMENT_URI}/pending-confirmation`, {
    method: 'GET',
    credentials: 'include',
    headers: {
      'Content-Type': 'application/json',
...
export const getPaymentById = async (id: string): Promise<Payment> => {
  const data = await http(`${BASE}${PAYMENT_URI}/${id}`, {
    method: 'GET',
    credentials: 'include',
    headers: {
      'Content-Type': 'application/json',
